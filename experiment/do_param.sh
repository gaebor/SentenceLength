...
then
  args="--iter 10000 --max 1000 --eta 0.5 --opt Adagrad --mae 1e-3"
  order=`echo $param | sed "s/o\([1-9]\+\)\.k\([0-9\.]\+\)/\1/"`
  k="`echo $param | sed "s/o\([1-9]\+\)\.k\([0-9\.]\+\)/\2/" | tr '.' ' '`"
  THEANO_FLAGS=device=cpu OMP_NUM_THREADS=1 python randomwalk.py \
        -o $order -k $k $args -f "$filename" &> >(tail -n 1)
else
  ...
  then
    ...
    then
      ...
      then
        ...
        ./sichel.m "$filename" 1000 "10^-4" 0.4 100  True      "$gamma" &> >(tail -n 1)
      else
        echo unknown model
      fi
    fi
